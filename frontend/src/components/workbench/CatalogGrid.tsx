      ...
      <section className="card-surface overflow-hidden">
        ...
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full min-w-[940px] border-collapse text-left">
            <thead className="bg-cream-soft text-[11px] uppercase tracking-[.09em] text-muted"><tr>
              <th scope="col" className="px-4 py-3"><input type="checkbox" aria-label="Select visible rows" checked={visible.length > 0 && visible.every((row) => selected.has(row.id))} onChange={() => onToggleAll(visible.map((row) => row.id))} /></th>
              <th scope="col" className="px-3 py-3">MPN / evidence</th><th scope="col" className="px-3 py-3">Description</th><th scope="col" className="px-3 py-3">Manufacturer</th><th scope="col" className="px-3 py-3">Classification</th><th scope="col" className="px-3 py-3">Status</th><th scope="col" className="px-4 py-3 text-right">Action</th>
            </tr></thead>
...
function ReplayBadge({ row }: { row: CatalogRow }) { const item = recognition(row); return item ? <span className={`mt-1.5 inline-flex rounded-full border px-2 py-0.5 text-[9.5px] font-semibold uppercase tracking-[.06em] ${item.tone}`}>{item.label}</span> : <span className="mt-1.5 block text-[11px] text-muted">No replay evidence</span>; }
...
function CatalogTableRow({ row, selected, result, busy, onToggle, onAnalyze, onOpen }: { row: CatalogRow; selected: boolean; result?: AnalyzedProduct; busy: boolean; onToggle: () => void; onAnalyze: () => void; onOpen: () => void }) {
  return <tr className="align-top hover:bg-cream-soft/45"><td className="px-4 py-4"><input type="checkbox" aria-label={`Select ${row.values.Mfg_Part_Num || `row ${row.sourceIndex}`}`} checked={selected} onChange={onToggle} disabled={row.status === "INVALID"} /></td><td className="px-3 py-4"><code className="text-[12.5px] font-semibold text-ink">{row.values.Mfg_Part_Num || "—"}</code><ReplayBadge row={row} /></td><td className="max-w-[260px] px-3 py-4 text-[12.5px] leading-relaxed text-muted">{row.values.Part_Desc || "—"}</td><td className="px-3 py-4 text-[12.5px] text-ink">{row.values.Part_Manuf || row.values.E1_Brand || "—"}</td><td className="px-3 py-4 text-[12.5px] text-muted">{result?.detail?.classification.family ?? "Not analyzed"}</td><td className="px-3 py-4"><ResultStatus result={result} />{!result ? <StageBadge status={row.status === "READY" ? "SUCCESS" : row.status === "REVIEW" ? "REVIEW" : "BLOCKED"} label={row.status} /> : null}{row.issues.length ? <p title={row.issues.join("; ")} className="mt-1.5 max-w-[150px] truncate text-[10.5px] text-muted">{row.issues.join(" · ")}</p> : null}</td><td className="px-4 py-4 text-right"><button type="button" disabled={row.status === "INVALID" || busy} onClick={result ? onOpen : onAnalyze} className="text-[12.5px] font-semibold text-green underline decoration-sage underline-offset-4 disabled:text-muted">{busy ? "Analyzing…" : result ? "Open result" : "Analyze"}</button></td></tr>;
}
...
function CatalogCard(props: Parameters<typeof CatalogTableRow>[0]) {
  ...
  return <li className="p-4"><div className="flex items-start justify-between gap-3"><label className="flex min-w-0 items-start gap-3"><input type="checkbox" checked={selected} onChange={onToggle} disabled={row.status === "INVALID"} className="mt-1" /><span className="min-w-0"><code className="block break-all text-[13px] font-semibold">{row.values.Mfg_Part_Num || "—"}</code><span className="mt-1 block text-[12px] text-muted">{row.values.Part_Manuf || row.values.E1_Brand || "No manufacturer"}</span></span></label><ResultStatus result={result} /></div><p className="mt-3 line-clamp-2 text-[12.5px] leading-relaxed text-muted">{row.values.Part_Desc || "No description"}</p><ReplayBadge row={row} /><div className="mt-4 flex items-center justify-between"><span>{!result ? <StageBadge status={row.status === "READY" ? "SUCCESS" : row.status === "REVIEW" ? "REVIEW" : "BLOCKED"} label={row.status} /> : null}</span><button type="button" disabled={row.status === "INVALID" || busy} onClick={result ? onOpen : onAnalyze} className="text-[12.5px] font-semibold text-green">{busy ? "Analyzing…" : result ? "Open result" : "Analyze"}</button></div></li>;
}
